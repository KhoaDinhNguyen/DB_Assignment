...
/*
DELIMITER $$
CREATE FUNCTION return_type(_staff_id CHAR(7))
RETURNS INT
DETERMINISTIC
BEGIN
	IF EXISTS (SELECT * FROM Coach WHERE Coach_id = _staff_id) THEN RETURN 0;
	ELSEIF EXISTS (SELECT * FROM Player WHERE Player_Id = _staff_id) THEN RETURN 1;
    ELSE RETURN 2;
    END IF; 
END$$
DELIMITER ;
